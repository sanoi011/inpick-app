...
## 완료된 작업 (2026-02-22) - 84B 외벽선 추출 + 가설 검증 전략 전환

### 전략 전환: 자동 도면 인식 → 단일 타입(84B) 가설 검증 우선
- **배경**: 도면 자동 인식 (Gemini Vision + YOLO + PyMuPDF 등 5소스 파이프라인)은 구축 완료했으나, 실제 정확도 검증과 워크플로우 E2E 입증이 먼저 필요
- **결정**: 84B 타입 하나를 기준으로 **도면 → 2D/3D 뷰어 → 물량산출 → 견적 → RFQ** 전체 워크플로우를 완전히 검증한 후 확장
- **원칙**: "도면 인식기능을 자동으로 하는건 현재 어려워 보이니 우선 한가지 타입으로 우리의 가설을 먼저 입증하자"

### 84B 외벽선 추출
- `public/floorplans/sample-84b-exterior.json` (신규) - 84B 외벽만 추출한 도면
  - 참조 이미지: `reference/1.도면 인식/84B 타입 벽선.jpg` (핑크/마젠타 하이라이트)
  - **12개 외벽 세그먼트** (단위세대 외곽 경계 완전 폐합)
  - 1개 현관문 (복도면 y=1.583)
  - 5개 외부 창문 (발코니 + 거실창 + 안방창)
  - 1개 전용공간 폴리곤 (단위세대 전체 풋프린트)

### 84B 외벽 좌표 (핑크 라인 → 벽 세그먼트)
```
단위세대 외곽 (시계방향, 좌상단 시작):
(0,0) → (2.176,0)         발코니2 상단
(2.176,0) → (2.176,1.583)  발코니2 우측
(2.176,1.583) → (6.727,1.583)  복도면 (북측)
(6.727,1.583) → (6.727,0)  발코니1 좌측
(6.727,0) → (12.068,0)     발코니1 상단
(12.068,0) → (12.068,1.583)  우측 상단
(12.068,1.583) → (11.08,1.583)  세대경계 단차
(11.08,1.583) → (11.08,6.331)   우측 중앙 (동측)
(11.08,6.331) → (12.068,6.331)  드레스룸 단차
(12.068,6.331) → (12.068,10.288)  우측 하단
(12.068,10.288) → (0,10.288)     하단 (남측, 거실창)
(0,10.288) → (0,0)         좌측 (서측)
```
- 총 풋프린트: ~112m² (전용 84m² + 공용/벽체 ~28m²)
- 특징: 우측에 2단 단차 (발코니 돌출 + 드레스룸/욕실 돌출)

### 다음 작업: 84B E2E 워크플로우 검증
1. 84B exterior JSON → FloorPlan2D 뷰어 렌더링 확인
2. 84B sample-84b.json (내벽 포함) → FloorPlan3D 뷰어 렌더링 확인
3. 물량산출 엔진: 84B 도면 기반 17개 공종 정확도 검증
4. 견적 생성 → PDF 내보내기 검증
5. RFQ 발송 → 사업자 수신 → 입찰 → 계약 체결 검증
6. 검증 완료 후 59㎡, 84A㎡ 타입 순차 확장
