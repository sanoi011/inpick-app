...
test.describe("Consumer Project Workflow", () => {
  test("project/new redirects to home tab", async ({ page }) => {
    ...
    // Should redirect to /project/[uuid]/design (default tab)
    await expect(page).toHaveURL(/\/project\/[a-f0-9-]+\/design/, { timeout: 10000 });
  });

  test("project design page renders", async ({ page }) => {
    await page.goto("/project/new");
    await expect(page).toHaveURL(/\/project\/[a-f0-9-]+\/design/, { timeout: 10000 });
    // Should show design-related UI
