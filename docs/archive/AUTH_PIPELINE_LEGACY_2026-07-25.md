...
3. Supabase 브라우저 클라이언트 생성 시 자동 PKCE code 교환을 시작한다.
4. 전역 인증 이벤트 리스너는 동기 반환하며 DB·API 후처리는 다음 task로 분리한다.
5. 콜백은 `initialize()` 완료를 기다리지 않고 세션 쿠키 생성을 확인한다. Arc에서
   중복 잔존하는 verifier 쿠키는 성공을 막는 기준으로 사용하지 않는다.
6. 콜백에서 `initialize()`, `getSession()`, `getUser()`를 성공 조건으로 기다리지
   않고 즉시 복귀한다.
