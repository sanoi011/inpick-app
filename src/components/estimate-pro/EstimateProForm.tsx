...
import {
  ...
  inferEstimateLineOptionId,
} from "@/lib/estimate-pro/line-options";
...
export default function EstimateProForm({
  ...
  const updateMaterialOption = (line: Row, optionId: string) =>
    setRows((current) => applyEstimateLineOption(current, line, optionId));
      ...
      <div className="p-4">
        ...
        {tab === "detail" && (
          <>
            ...
            <DetailTab sheet={detailSheet} groupBy={groupBy} setGroupBy={setGroupBy} expanded={expanded} toggleGroup={toggleGroup} updateRow={updateRow} updateMaterialOption={updateMaterialOption} deleteRow={deleteRow} precedingByTrade={precedingByTrade} role={role} />
          </>
        )}
        {tab === "schedule" && (
          <ScheduleTab
        ...
        <tbody>
          {rows.map((r: any, index: number) => (
            <tr key={r.trade} className="border-b border-slate-50 hover:bg-black/[0.025]">
              <td className="px-3 py-1.5 text-slate-700">{String(index + 1).padStart(4, "0")}. {r.trade}</td>
...
/* 4. 세부내역서 (편집) */
function DetailTab({ sheet, groupBy, setGroupBy, expanded, toggleGroup, updateRow, updateMaterialOption, deleteRow, precedingByTrade, role }: any) {
  return (
              ...
              <>
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-sm border-collapse">
                    ...
                    <tbody>
                      {g.lines.map((l: Row) => (<EditRow key={l.id} l={l} updateRow={updateRow} updateMaterialOption={updateMaterialOption} deleteRow={deleteRow} role={role} />))}
...
function EditRow({ l, updateRow, updateMaterialOption, deleteRow, role }: { l: Row; updateRow: any; updateMaterialOption: (line: Row, optionId: string) => void; deleteRow: any; role: "owner" | "bidder" }) {
  const hasMat = l.brand !== "-";
  const editable = role === "bidder" && !l.isWorkPackage;
  const materialOptions = getEstimateLineOptions(l);
  const selectedMaterialOption = inferEstimateLineOptionId(l, materialOptions);
    ...
    <tr className="border-b border-zinc-50 hover:bg-zinc-50/60">
      ...
      <td className="px-2 py-1.5">
        ...
        {materialOptions.length > 0 && (
          <label className="mt-1.5 flex max-w-[260px] items-center gap-2 rounded-lg border border-zinc-200 bg-white px-2.5 py-1.5 shadow-sm transition focus-within:border-zinc-400">
            <span className="shrink-0 text-[10px] font-bold text-zinc-400">
              옵션
            </span>
            <select
              aria-label={`${l.room} ${l.part} 옵션`}
              value={selectedMaterialOption}
              onChange={(event) => updateMaterialOption(l, event.target.value)}
              className="min-w-0 flex-1 cursor-pointer bg-transparent text-[12px] font-bold text-zinc-700 outline-none"
            >
      ...
      <div className="mb-5 flex flex-wrap items-start justify-between gap-3">
        <div>
          <div className="flex flex-wrap items-center gap-2">
            ...
            <span className="rounded-full bg-gradient-to-r from-blue-200 to-sky-100 px-2.5 py-1 text-[11px] font-bold text-slate-700 shadow-sm shadow-blue-100">
  ...
  return (
    <div className="group flex items-center gap-2">
      ...
      <div className="relative h-9 min-w-[180px] flex-1 overflow-hidden rounded-lg bg-blue-50/60 ring-1 ring-inset ring-blue-100/50">
        ...
        <div
          className="absolute inset-y-1 flex min-w-[24px] items-center overflow-hidden whitespace-nowrap rounded-md px-2 text-[11px] font-bold text-slate-700 shadow-sm shadow-blue-100/70 transition-[filter] group-hover:brightness-[1.02]"
          style={{
