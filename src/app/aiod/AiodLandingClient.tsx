...
export default function AiodLandingClient() {
  ...
  return (
    <>
      ...
      {/* Hero */}
      <section className="relative overflow-hidden bg-burgundy pt-32 pb-24 lg:pt-40 lg:pb-32">
        ...
        <div className="mx-auto max-w-7xl px-6 lg:px-8">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="flex flex-col items-start gap-6">
            <div className="inline-flex items-center gap-2 rounded-full border border-white/15 bg-white/[0.06] px-3 py-1 text-xs font-medium text-apricot-300/80 backdrop-blur">
            ...
            <p className="max-w-2xl text-base leading-relaxed text-apricot-300/80 sm:text-lg">
            ...
            <div className="mt-2 flex flex-wrap items-center gap-3">
              ...
              <motion.a href="#business" whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}
                className="inline-flex items-center gap-2 rounded-full border border-white/20 bg-white/[0.06] px-5 py-2.5 text-sm font-medium text-white backdrop-blur hover:bg-white/10">
                사업 영역 보기
              </motion.a>
            </div>
          </motion.div>
          ...
          <div className="grid gap-12 lg:grid-cols-2 lg:gap-20">
            <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5 }}>
              ...
              <p className="mt-6 text-base leading-relaxed text-ink-60">
                ...
                <strong className="text-ink"> 현장의 언어로 학습된 AI</strong> 라는 결론에 도달했습니다.
              </p>
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: 0.1 }}
              ...
              {[
                ...
                <div key={stat.k} className="rounded-[28px] border border-primary-100 bg-apricot-100 p-6">
                  <p className="text-xs font-medium uppercase tracking-wider text-ink-40">{stat.k}</p>
                  <p className="mt-2 text-3xl font-bold text-ink">{stat.v}</p>
                  <p className="mt-1 text-xs text-ink-40">{stat.sub}</p>
                </div>
              ))}
            </motion.div>
          </div>
      ...
      {/* Business areas */}
      <section id="business" className="bg-apricot-100 py-20 lg:py-28">
        <div className="mx-auto max-w-7xl px-6 lg:px-8">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5 }}
            className="mx-auto max-w-2xl text-center">
            <p className="text-sm font-semibold uppercase tracking-wider text-primary-500">Business</p>
            <h2 className="mt-3 text-3xl font-bold tracking-tight text-ink sm:text-4xl">사업 영역</h2>
            <p className="mt-4 text-base text-ink-60">하나의 견적 플랫폼에서 시작해, 건축 전반의 AI 자동화로 확장합니다.</p>
          </motion.div>
          <div className="mt-14 grid gap-6 lg:grid-cols-3">
            {BUSINESS_AREAS.map((area, idx) => (
              <motion.div key={area.title}
                ...
                className="group flex flex-col rounded-[28px] border border-primary-100 bg-white p-8 hover:border-primary-300 hover:shadow-lg transition-all">
                <div className="flex items-center justify-between">
                  ...
                  <span className="rounded-full bg-primary-100 px-2.5 py-1 text-xs font-medium text-ink-60">{area.status}</span>
                </div>
                <p className="mt-6 text-xs font-semibold uppercase tracking-wider text-primary-500">{area.tag}</p>
                <h3 className="mt-1 text-xl font-bold text-ink">{area.title}</h3>
                <p className="mt-3 text-sm leading-relaxed text-ink-60">{area.desc}</p>
              </motion.div>
            ))}
          </div>
        </div>
      ...
      <section id="principles" className="bg-white py-20 lg:py-28">
        <div className="mx-auto max-w-7xl px-6 lg:px-8">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5 }}
            ...
            <h2 className="mt-3 text-3xl font-bold tracking-tight text-ink sm:text-4xl">우리가 지키는 3가지</h2>
          </motion.div>
          <div className="mt-14 grid gap-8 lg:grid-cols-3 lg:gap-10">
            {PRINCIPLES.map((p, idx) => (
              <motion.div key={p.title}
                initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: idx * 0.1 }}
                className="relative rounded-[28px] border-l-4 border-primary-500 bg-apricot-100 p-8">
      ...
      {/* CTA + Company Info */}
      <section id="contact" className="bg-gradient-to-b from-burgundy to-wine-700 py-20 lg:py-28">
        <div className="mx-auto max-w-7xl px-6 lg:px-8">
          <div className="grid gap-12 lg:grid-cols-2 lg:gap-16 lg:items-center">
            <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5 }}>
              ...
              <p className="mt-4 text-base leading-relaxed text-apricot-300/80">
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: 0.1 }}
              className="rounded-[28px] border border-white/15 bg-white/[0.06] p-8 backdrop-blur">
              <p className="text-xs font-semibold uppercase tracking-wider text-primary-300">회사 정보</p>
              <dl className="mt-4 flex flex-col gap-3">
                {COMPANY_INFO.map((info) => (
                  <div key={info.k} className="flex items-start justify-between gap-4 border-b border-white/15 pb-3 last:border-0 last:pb-0">
                    <dt className="text-sm text-apricot-300/70">{info.k}</dt>
