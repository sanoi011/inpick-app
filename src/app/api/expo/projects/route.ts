...
export async function GET() {
  ...
  const { data, error } = await supabase
    .from("expo_projects")
    .select(
      "id, title, area_input, area_unit, footprint, confirmed_dimensions, scene, concept_image_url, brand, event, official_services, estimate_overrides, proposal, concept_images, contract_prep, print_items, quick_fields, client_decision, updated_at",
    )
    .eq("status", "draft")
