  ...
  return (
    <div className="max-w-3xl mx-auto px-6 py-8">
      <div className="flex items-center justify-between mb-6">
        ...
        {unreadCount > 0 && (
          <button onClick={handleMarkAllRead} className="flex items-center gap-1 text-xs text-blue-600 font-medium hover:text-blue-800">
        ...
        {FILTER_TABS.map((tab) => (
          <button
            ...
            className={`px-3 py-1.5 rounded-full text-xs font-medium whitespace-nowrap transition-colors ${
              activeFilter === tab ? "bg-blue-600 text-white" : "bg-white border border-gray-200 text-gray-600 hover:bg-gray-50"
            }`}
          >
      ...
      {loading ? (
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-8 text-center">
          ...
          <button onClick={loadNotifications} className="flex items-center gap-1 mx-auto text-sm text-blue-600 font-medium"><RefreshCw className="w-3.5 h-3.5" /> 재시도</button>
        </div>
      ) : filtered.length === 0 ? (
        <div className="bg-white border border-gray-200 rounded-xl p-12 text-center">
          <Bell className="w-10 h-10 text-gray-300 mx-auto mb-3" />
          <p className="text-sm text-gray-500">{activeFilter === "전체" ? "알림이 없습니다" : `${activeFilter} 알림이 없습니다`}</p>
        </div>
      ) : (
        <div className="bg-white border border-gray-200 rounded-xl overflow-hidden divide-y divide-gray-50">
          {filtered.map((n) => (
            <Link key={n.id} href={n.link || "#"} onClick={() => !n.isRead && handleMarkRead(n.id)}
              className={`px-5 py-4 flex items-start gap-3 hover:bg-gray-50 border-l-4 transition-colors ${CONSUMER_NOTIFICATION_PRIORITY_COLORS[n.priority]} ${!n.isRead ? "bg-blue-50/30" : ""}`}
            >
